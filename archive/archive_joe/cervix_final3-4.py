...
def get_signatures(train_set_, train_set_target_):
      ...
      for l in range(2,7):
        ...
        s1 = pymrmre.mrmr_ensemble(features=train_set_.drop(columns=['cohort']), targets = train_set_target_.drop(columns=['time','figo2018']),
                                    solution_length=l, solution_count=1, fixed_features=['shape_VoxelVolume', 'figo2018'])
        ...
        if l == 2:
          choosen = ['event','time', 'figo2018', 'shape_VoxelVolume']
          # volume dichotomized...
          dico_choosen = ['event', 'time', 'figo2018', "vol_dico"]
        else:
...
save_obj(results,f'/Users/josephmarsilla/Google Drive/notebooks/Cervix/Data/NORONLY-VOLSTAGEREGNORM_{date}_{max_iter}.pkl')
