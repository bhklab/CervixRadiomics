...
def normalize(train_set_, test_set_, cohort_norm=False, ignore = ['cohort', 'figo2018', 'event', 'time', 'shape_VoxelVolume']):
...
def get_signatures(train_set_, test_set_, train_set_target_,  test_set_target_):
      ...
      for l in range(2,7):
        ...
        s1 = pymrmre.mrmr_ensemble(features=train_set_.drop(columns=['shape_VoxelVolume']), targets = train_set_target_.drop(columns=['time','figo2018']),
        ...
        if l == 2:
          ...
          choosen = ['event','time', 'cohort', 'figo2018', 'Signature']
          dico_choosen = ['event','time', 'cohort', 'figo2018', "high_signature"]
...
save_obj(results,f'/Users/josephmarsilla/Google Drive/notebooks/Cervix/Data/-STAGEREGNORM_{date}_{max_iter}.pkl')
