...
def get_signatures(train_set_, test_set_, train_set_target_,  test_set_target_):
      ...
      for l in range(1,6):
        ...
        s1 = pymrmre.mrmr_ensemble(features=train_set_.drop(columns=['shape_VoxelVolume', 'cohort']), targets = train_set_target_.drop(columns=['time','figo2018']),
                                    solution_length=l, solution_count=1, fixed_features=['figo2018'])
        ...
        if l == 1:
          ...
          dico_choosen = ['event','time', 'figo2018', "high_signature"]
          # choosen = ['event','time', 'figo2018', 'Signature']
...
save_obj(results,f'/Users/josephmarsilla/Google Drive/notebooks/Cervix/Data/NORONLY-STAGEREGNORM_{date}_{max_iter}.pkl')
