...
def get_signatures(train_set_, test_set_, train_set_target_,  test_set_target_):
      ...
      for l in range(1,6):
        ...
        s1 = pymrmre.mrmr_ensemble(features=train_set_.drop(columns=['figo2018', 'cohort']), targets = train_set_target_.drop(columns=['time','figo2018']),
                                    solution_length=l, solution_count=1, fixed_features=['shape_VoxelVolume'])

        doc['features'] = list(s1.iloc[0])[0][1:]
        doc['num_features'] = l - 1 # in addition to volume...
        ...
        if l == 1:
          ...
          dico_choosen = ['event', 'time', 'high_signature']
        else:
          choosen = ['event','time', 'shape_VoxelVolume', 'Signature']
          dico_choosen = ['event','time', 'vol_dico', "high_signature"]
...
save_obj(results,f'/Users/josephmarsilla/Google Drive/notebooks/Cervix/Data/NORONLY-VOLREGNORM_{date}_{max_iter}.pkl')
