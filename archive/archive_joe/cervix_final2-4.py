...
def get_signatures(train_set_, train_set_target_):
      ...
      for l in range(3,8):
        ...
        s1 = pymrmre.mrmr_ensemble(features=train_set_, targets = train_set_target_.drop(columns=['time','figo2018']),
                                    solution_length=l, solution_count=1, fixed_features=['cohort','shape_VoxelVolume', 'figo2018'])
        ...
        if l == 3:
          choosen = ['event','time', 'cohort', 'figo2018', 'shape_VoxelVolume']
          # volume dichotomized...
          dico_choosen = ['event', 'time', 'cohort', 'figo2018', "vol_dico"]
        else:
          ...
          choosen = ['event','time', 'figo2018', 'shape_VoxelVolume', 'Signature']
          # everything dicotomized including volume...
          dico_choosen = choosen = ['event','time', 'figo2018', "vol_dico", "high_signature"]
...
for k in range(max_iter):
  ...
  try:
      lab = get_signatures(train_set_, train_set_target_)
...
save_obj(results,f'/Users/josephmarsilla/Google Drive/notebooks/Cervix/Data/-VOLSTAGEREGNORM_{date}_{max_iter}.pkl')
