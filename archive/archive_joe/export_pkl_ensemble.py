...
for i in range(len(a)):
    for j in range(len(a[0])):
        ...
        # don't add 1 if just Norway on it's own... don't need to add cohort in the modeling...
        test_plots[j].append(a[i][j+1]['test_ci'])
        train_plots[j].append(a[i][j+1]['train_ci'])
        # comment out if dicotomized data not in dic...
...
for i in range(len(train_plots)):
    ...
    # comment out if no DICHOTOMOIZED DATA...
