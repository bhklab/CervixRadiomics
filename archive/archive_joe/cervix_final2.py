...
def get_signatures(train_set_, test_set_, train_set_target_,  test_set_target_):
      ...
      for l in range(2,7):
        ...
        s1 = pymrmre.mrmr_ensemble(features=train_set_.drop(columns=['figo2018']), targets = train_set_target_.drop(columns=['time','figo2018']),
                                    solution_length=l, solution_count=1, fixed_features=['cohort', 'shape_VoxelVolume'])
        ...
        if l == 2:
          choosen = ['event','time', 'cohort', 'shape_VoxelVolume']
          # choosen = ['event','time', 'figo2018']
          # initially both are the same...
          dico_choosen = ['event', 'time', 'cohort', 'high_signature']
        else:
          choosen = ['event','time', 'cohort', 'shape_VoxelVolume', 'Signature']
          dico_choosen = ['event','time', 'cohort', 'vol_dico', "high_signature"]
...
save_obj(results,f'/Users/josephmarsilla/Google Drive/notebooks/Cervix/Data/-VOLREGNORM_{date}_{max_iter}.pkl')
