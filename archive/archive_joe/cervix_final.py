...
for k in range(max_iter):
  ...
  # set cohort norm to true...
  # set cohort norm to true...
  # cohort_norm = True
  ignore = ['cohort', 'event', 'time'] # 'figo2018'
  if cohort_norm:
  ...
  for l in range(2,7):
    ...
    s1 = pymrmre.mrmr_ensemble(features=train_set_, targets = train_set_target_.drop(columns=['time','figo2018']),
                                solution_length=l, solution_count=1, fixed_features=['cohort', 'figo2018'])
    ...
    a = list(train_mrmr)
    choice = ['shape_VoxelVolume', 'figo2018']
    ...
    print('median_value:', b[choose].median())
    ...
    if l == 2:
      # choosen = ['event','time', 'shape_VoxelVolume']
      choosen = ['event','time', 'figo2018']
      # initially both are the same...
      dico_choosen = ['event', 'time', 'figo2018']
    else:
      # choosen = ['event','time', 'shape_VoxelVolume', 'Signature']
      # dico_choosen = choosen = ['event','time', 'shape_VoxelVolume', "high_signature"]
      # signature including FIGO score...
    ...
    test_signature1 = -cphf.predict_partial_hazard(btest[choosen]).values

    # print(test_signature)
    c1 = concordance_index(btest['time'], test_signature1, btest['event'])
    doc['test_ci'] = c1
    print('Test CI: ', c1)
...
save_obj(results,f'/Users/josephmarsilla/Google Drive/notebooks/Cervix/Data/R4_normfigo_HIGHSTAGEREGNORM_{date}_{max_iter}.pkl')
